...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets LegnaCode handle permission prompts automatically — LegnaCode checks each tool call for risky actions and prompt injection before executing. Actions LegnaCode identifies as safe are executed, while actions LegnaCode identifies as risky are blocked and LegnaCode may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. LegnaCode can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
