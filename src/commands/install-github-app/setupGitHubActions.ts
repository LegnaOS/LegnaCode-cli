...
async function createWorkflowFile(
  ...
  if (createFileResult.code !== 0) {
    if (
      ...
      throw new Error(
        `Failed to create workflow file ${workflowPath}: A LegnaCode workflow file already exists in this repository. Please remove it first or update it manually.`,
      )
    }
    ...
    if (!skipWorkflow) {
      ...
      if (selectedWorkflows.includes('claude')) {
        workflows.push({
          ...
          message: 'LegnaCode PR Assistant workflow',
        })
      }
      ...
      if (selectedWorkflows.includes('claude-review')) {
        workflows.push({
          ...
          message: 'LegnaCode Review workflow',
        })
      }
