      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[LegnaCode in Chrome] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[LegnaCode in Chrome] No compatible browser found')
    return false
  }
...
function getSocketName(): string {
  // NOTE: This must match the one used in the LegnaCode in Chrome MCP
