...
/**
 * Builds a deep link URL for LegnaCode Desktop to resume a CLI session.
...
/**
 * Check if LegnaCode Desktop app is installed.
 * On macOS, checks for /Applications/LegnaCode.app.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for LegnaCode.app in /Applications
    return pathExists('/Applications/LegnaCode.app')
...
/**
 * Detect the installed LegnaCode Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/LegnaCode.app/Contents/Info.plist',
...
/**
 * Build and open a deep link to resume the current session in LegnaCode Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'LegnaCode Desktop is not installed. Install it from https://claude.ai/download',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open LegnaCode Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
