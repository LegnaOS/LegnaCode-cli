 ...
 * notice which directory — and therefore which LEGNA.md — was loaded.
 */
 ...
 *
 * Always shows the working directory so the user can see which LEGNA.md
 ...
 * and whether its LEGNA.md may be stale relative to upstream.
 */
export function buildDeepLinkBanner(info: DeepLinkBannerInfo): string {
  ...
  if (info.repo) {
    ...
    lines.push(
      `Resolved ${info.repo} from local clones · last fetched ${age}${stale ? ' — LEGNA.md may be stale' : ''}`,
    )
  }
