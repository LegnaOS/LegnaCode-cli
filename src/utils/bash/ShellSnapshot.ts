...
/**
 * Generates LegnaCode specific snapshot content
...
async function getSnapshotScript(
  ...
  // Generate the user content and LegnaCode content
 ...
 *
 * This function is a critical part of LegnaCode CLI's shell integration strategy. It:
 *
 ...
 *
 * This approach allows LegnaCode CLI to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with LegnaCode defaults only`,
        )
      }
