...
export function getMacOsKeychainStorageServiceName(
  ...
  return `LegnaCode${getOauthConfig().OAUTH_FILE_SUFFIX}${serviceSuffix}${dirHash}`
}

/**
 * Get the legacy keychain service name for backwards compatibility with Claude Code.
 */
export function getLegacyMacOsKeychainStorageServiceName(
  serviceSuffix: string = '',
): string {
  const configDir = getClaudeConfigHomeDir()
  const isDefaultDir = !process.env.CLAUDE_CONFIG_DIR
