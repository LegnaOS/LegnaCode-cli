...
export async function getClaudeDesktopConfigPath(): Promise<string> {
  ...
  if (!SUPPORTED_PLATFORMS.includes(platform)) {
    throw new Error(
      `Unsupported platform: ${platform} - LegnaCode Desktop integration only works on macOS and WSL.`,
    )
  }
  ...
  try {
    ...
    try {
      ...
      // Look for LegnaCode Desktop config in each user directory
  ...
  throw new Error(
    'Could not find LegnaCode Desktop config file in Windows. Make sure LegnaCode Desktop is installed on Windows.',
  )
}
...
> {
  if (!SUPPORTED_PLATFORMS.includes(getPlatform())) {
    throw new Error(
      'Unsupported platform - LegnaCode Desktop integration only works on macOS and WSL.',
    )
  }
  try {
