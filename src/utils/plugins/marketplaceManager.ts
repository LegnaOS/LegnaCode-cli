/**
 * Marketplace manager for LegnaCode plugins
 *
...
export async function loadKnownMarketplacesConfigSafe(): Promise<KnownMarketplacesConfig> {
  try {
    ...
    // corrupted user config isn't a LegnaCode bug, shouldn't hit the error file.
    return {}
  }
}
...
async function cacheMarketplaceFromUrl(
  ...
  const headers = {
    ...
    'User-Agent': 'LegnaCode-Plugin-Manager',
  }
    ...
    if (
      ...
      throw new Error(
        `Marketplace "${name}" has a relative source path (${entry.source.path}) ` +
          ...
          `LegnaCode version. Run 'claude marketplace remove ${name}' and ` +
