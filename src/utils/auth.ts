...
/**
 * CCR and LegnaCode Desktop spawn the CLI with OAuth and should never fall back
...
export function getAnthropicApiKeyWithSource(
  ...
  // Always check for direct environment variable when the user ran legna --print.
  ...
  (): { key: string; source: ApiKeySource } | null => {
    ...
    if (process.platform === 'darwin') {
      ...
      if (prefetch) {
        ...
        // Fallback: try legacy "Claude Code" keychain entry
        const legacyServiceName = getLegacyMacOsKeychainStorageServiceName()
        if (legacyServiceName !== storageServiceName) {
          try {
            const result = execSyncWithDefaults_DEPRECATED(
              `security find-generic-password -a $USER -w -s "${legacyServiceName}"`,
            )
            if (result) {
              return { key: result, source: '/login managed key' }
            }
...
export function getSubscriptionName(): string {
  ...
  switch (subscriptionType) {
    case 'enterprise':
      return 'LegnaCode Enterprise'
    case 'team':
      return 'LegnaCode Team'
    case 'max':
      return 'LegnaCode Max'
    case 'pro':
      return 'LegnaCode Pro'
    default:
      return 'LegnaCode API'
  }
}
